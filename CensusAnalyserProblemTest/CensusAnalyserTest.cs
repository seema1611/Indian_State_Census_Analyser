...
namespace CensusAnalyserProblemTest
{
    public class CensusAnalyserTest
    {
        ...
        //TC-11.1
        [Test]
        public void GivenUsCensusAndIndianCensusCSVFile_Whensorted_ShouldReturnsMostPopulousState()
        {
        ...
        //TC-11.2
        [Test]
        public void GivenUsCensusAndIndianCensusCSVFile_Whensorted_ShouldReturnsLeastPopulousState()
        {
            var usRecord = censusAdapter.LoadCensusData<USCensus>(Country.US, usCensusDataHeaders, usCSVFilePath);
            string usSortedList = censusAdapter.SortAndConvertCensusToJson(usRecord, DTO.US, SortBy.POPULATION_DENSITY, SortOrder.DESCENDING);
            List<USCensus> usCensusSortedList = JsonConvert.DeserializeObject<List<USCensus>>(usSortedList);
            ...
            Assert.AreEqual("Arunachal Pradesh", censusAdapter.GetMostPopulousStateBetweenBoth(indianCensusSortedList[indianCensusSortedList.Count - 1], usCensusSortedList[0]));
        }
    }
}